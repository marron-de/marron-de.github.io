<?php
/**
 * page file : /theme/THEME_NAME/page/aboutus.html.php
 */
if (!defined('_EYOOM_')) exit;
?>


<link rel="stylesheet" href="../theme/eb4_basic/page/detail_css.css">
<style>
    .line-156 {
        width:182px;
    }
</style>
<div class="sub-page page-class detail page-detail">
    <div class="main-container" id="#reference-object" data-aos="fade">
        <div class="frame-c">
            <div class="container">
                <div class="frame-d">
                    <span class="sosok-hagi-e">수업듣기 >
                    </span>
                    <span class="dapsa">국제학교 중/고등 학과목 밀착과외</span>
                </div>
            </div>
        </div>
        <div class="container target-section main">
            <div class="holding_cont">
                <div class="frame-16">
                    <div class="frame-17">
                        <div class="frame-18">
                            <div class="frame-19">
                                <span class="dapsa-jeong-mi-deul-i-deul-eo-gap-ni-da">국제학교 중/고등 학과목 밀착과외</span>
                            </div>
                            <div class="frame-1a">
                                <div class="tooltip">
									<button type="button" class="close_btn"></button>
                                    <div class="content">
                                        <span class="supporting-text">마음에 드는 서비스를 공유해보세요</span>
                                    </div>
                                    <div class="div"></div>
                                </div>
                                <div class="frame-1b">
                                    <div class="frame-1c">
                                        <div class="share"></div>
                                        <div class="favorite"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- <span class="muryo-yuhag-seobiseu-wa-ibsi-keon-sel-ting-kkaji-dayang-han-seobiseu-je-gong">원어민이 풀어주고, 원어민이 리뷰하는 듀오링고 DET 수업</span> -->
                    </div>
                    <div class="frame-1d">
                        <div class="tags-1e">
                            <span class="best">BEST</span>
                        </div>						
                        <div class="tag">
                            <span>체험반</span>
                        </div>
                        <div class="tag">
                            <span>정규반</span>
                        </div>

                        <!-- <div class="keulraeseu">
                            <span class="yeong-eo">1:1 클래스</span>
                        </div>
                        <div class="frame-1f">
                            <span class="text-16">영어</span>
                        </div> -->
                    </div>
                </div>
                <div class="section-c tabs">
                    <div class="ms title_area class stickybox" id="row-1">
                        <div class="tab sticky">
                            <ul class="tab_tit">
                                <li><a href="<?php echo G5_URL ?>/page/?pid=class_detail09" class="show">CLASS소개</a></li>
                                <li><a href="<?php echo G5_URL ?>/page/?pid=class_detail09_01">비용</a></li>
                                <li><a href="<?php echo G5_URL ?>/page/?pid=class_detail09_02">선생님</a></li>
                                <li><a href="<?php echo G5_URL ?>/page/?pid=class_detail09_03">자주 묻는 질문</a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="classDetail_list tabs_cont">
                        <ul class="panel">
                            <li id="content-1" style="display:block;">
                                <div class="frame-25 class_sec1">
                                    <div class="frame-section">
                                        <div class="frame-27">
                                            <span class="frame-title">국제학교 중/고등 학과목 밀착과외 수업 소개</span>
                                            <!-- <div class="tags-28">
                                                <span class="best-29">몽선생 CLASS</span>
                                            </div> -->
                                        </div>
                                        <span class="field-trip-service">
                                            국제학교 입학 시 보게 되는 CAT4, MAP테스트 그리고 인터뷰 준비까지 실제 시험 유형에 맞춘 모의 테스트를 통해 국제학교 입학 준비를 완벽하게 할 수 있습니다. 초/중/고 학년별 그리고 희망하는 학교에 맞추어 1:1로 지도하여 성공적인 입학을 도와드립니다.
                                        </span>
                                    </div>
                                </div>
                                <div class="line-2a"></div>
                                <div class="frame-25 class_sec2">
                                    <div class="frame-section">
                                        <div class="frame-27">
                                            <span class="frame-title">몽선생 국제학교 중/고등 학과목 밀착과외 수업이 특별한 4가지</span>
                                        </div>
                                        <div class="class_benefit">
                                            <div class="img_area">
                                                <img src="<?php echo EYOOM_THEME_URL; ?>/img/sub/img_cls_benefit20.png" alt="">
                                                <span>완벽한 시험 대비가능!</span>
                                                <p>
                                                    해외학교 교직원 출신!<br/>
                                                    공식 인증된 몽선생에서<br/>
                                                    검증된 몽선생이 제작한<br/>
                                                    실전 모의테스트 학습 제공
                                                </p>
                                            </div>
                                            <ul class="bnf_list">
                                                <li>
                                                    <span>01</span>
                                                    <p>
                                                        <b>어디에도 없어요!</b><br/>
                                                        자체제작 문제집 제공 
														<br class="pc">
														(CAT4 50/Map 50문제 세트)
                                                    </p>
                                                </li>
                                                <li>
                                                    <span>02</span>
                                                    <p>
                                                        <b>짧고 효과적인 학습 프로그램!</b><br/>
                                                        맞춤 6강 수업으로 테스트를
														<br class="pc"> 함께 준비해요
                                                    </p>
                                                </li>
                                                <li>
                                                    <span>03</span>
                                                    <p>
                                                        <b>검증된 선생님의 1:1 맞춤수업!</b><br/>
                                                        실전형 모의 입학 테스트 및
														<br class="pc"> 상세 피드백 제공
                                                    </p>
                                                </li>
                                                <li>
                                                    <span>04</span>
                                                    <p>
                                                        <b>체험반부터 정규반까지!</b><br/>
                                                        다양한 수업 방식을 제공합니다
                                                    </p>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                                <div class="line-2a"></div>
                                <div class="frame-25 class_sec3">
                                    <div class="frame-section">
                                        <div class="frame-27">
                                            <span class="frame-title">국제학교 중/고등 학과목 밀착과외 수업 커리큘럼</span>
                                        </div>
                                        <div class="class_curri">
                                            <ul class="curri_list">
                                                <li>
                                                    <span>
                                                        체험반<br/>
                                                        <small>(1회 수업)</small>
                                                    </span>
                                                    <div class="text_area">
                                                        <span class="t_top">
                                                            1회 체험반
                                                            <small>총 50분</small>
                                                        </span>
                                                        <div class="t_txt">
                                                            <span>테스트 문제 맛보기</span>
                                                            <p class="lst">‧ CAT4, MAP 연습문제 풀이</p>
                                                            <p class="lst">‧ 50가지 질문 유형 파악하기</p>
                                                            <p class="lst">‧ 인터뷰 예상질문 알아보기</p>
                                                        </div>
                                                    </div>
                                                </li>
                                                <li>
                                                    <span>
                                                        정규반<br/>
                                                        <small>(6회 수업)</small>
                                                    </span>
                                                    <div class="text_area">
                                                        <span class="t_top">
                                                            6회 정규반
                                                            <small>총 300분 (회당 50분)</small>
                                                        </span>
                                                        <div class="t_txt">
                                                            <span>테스트 실전 연습 및 집중 실력 향상</span>
                                                            <p class="lst">‧ CAT4, MAP, CEPT 대비 수업</p>
                                                            <p class="lst">‧ 어휘력, 라이팅 실력 향상 집중</p>
                                                            <p class="lst">‧ 인터뷰 연습 및 피드백 제공</p>
                                                        </div>
                                                    </div>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                                <div class="line-2a"></div>
                                <div class="class_curri">
                                    <div class="tit_area">
                                        <span>국제학교 중/고등 학과목 밀착과외 강의주제</span>
                                    </div>
                                    <div class="msection faq_area">
                                        <div class="container">
                                            <div class="faq-box">
                                                <ul class="faq_list">
                                                    <li>
                                                        <div class="question">
                                                            <p>정규반 1강</p>
                                                            <span>실전 입학 테스트</span>
                                                        </div>
                                                        <div class="answer">
                                                            <p>실제 유형 기반 모의시험 훈련 및 문제 해결 전략 습득</p>
                                                        </div>
                                                    </li>
                                                    <li>
                                                        <div class="question">
                                                            <p>정규반 2강</p>
                                                            <span>입학 테스트 인터뷰 대비 1</span>
                                                        </div>
                                                        <div class="answer">
                                                            <p>기본 인터뷰 응답 연습 및 자연스러운 자기표현 능력 향상</p>
                                                        </div>
                                                    </li>
                                                    <li>
                                                        <div class="question">
                                                            <p>정규반 3강</p>
                                                            <span>입학 테스트 인터뷰 대비 2</span>
                                                        </div>
                                                        <div class="answer">
                                                            <p>의견 제시형 질문 대응력 강화 및 논리적 표현 훈련</p>
                                                        </div>
                                                    </li>
                                                    <li>
                                                        <div class="question">
                                                            <p>정규반 4강</p>
                                                            <span>입학 테스트 라이팅 대비 1</span>
                                                        </div>
                                                        <div class="answer">
                                                            <p>문단 구성 연습 및 기본 에세이 구조 이해</p>
                                                        </div>
                                                    </li>
                                                    <li>
                                                        <div class="question">
                                                            <p>정규반 5강</p>
                                                            <span>입학 테스트 라이팅 대비 2</span>
                                                        </div>
                                                        <div class="answer">
                                                            <p>실전형 글쓰기 훈련 및 표현력, 전개력 향상</p>
                                                        </div>
                                                    </li>
                                                    <li>
                                                        <div class="question">
                                                            <p>정규반 6강</p>
                                                            <span>CAT4 / MAP 테스트 대비</span>
                                                        </div>
                                                        <div class="answer">
                                                            <p>핵심 문제 유형 분석 및 시간 관리 전략 학습</p>
                                                        </div>
                                                    </li>
                                                    <li>
                                                        <div class="question">
                                                            <p>체험반 1강</p>
                                                            <span>CAT4 / MAP 테스트 대비</span>
                                                        </div>
                                                        <div class="answer">
                                                            <p>핵심 문제 유형 분석 및 시간 관리 전략 학습</p>
                                                        </div>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="guide">
                                                <ul>
                                                    <li>* 위위 수업 내용은 예시 내용이며, 실제 수업시 학생들의 실력에 따라 내용은 개별 맞춤으로 진행될 수 있습니다.</li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="class_intro">
                                    <!-- <div class="img_area">
                                        <img src="<?php echo EYOOM_THEME_URL; ?>/image/sub/img_class_intro01.png" alt="">
                                        <p>교재 자세히보기</p>
                                    </div> -->
                                    <div class="text_area">
										<p class="title">
											<span class="color">국제학교 중/고등 학과목 밀착과외</span>
											 수업 학습 목표
										</p>
										<div class="descbox">
											<p>1. CAT4, MAP테스트 등 실전 문제 미리 파악하기</p>
											<p>2. 학교별 출제 문제 유형을 반영하여 반복 훈련하기</p>
											<p>3. 학생 개별 약점을 진단하고, 보강이 필요한 영역을 집중적으로 강화</p>
										</div>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="floating_cont sidebar">
                <div class="floating_wrap">
                    <div class="sseom-neol">
                        <div class="mal-lei-si-a-dae-hag cls01_09"></div>
                    </div>
                    <div class="frame-14f">
                        <div class="frame-150">
                            <div class="frame-151">
                                <div class="frame-152">
                                    <span class="day-paid">국제학교 중/고등 학과목 밀착과외</span>
                                </div>
                                <div class="group-154">
                                    <div class="line-155"></div>
                                    <div class="line-156"></div>
                                </div>
                            </div>
                            <div class="frame-157">
                                <div class="frame-158">
                                    <div class="frame-159">
                                        <span class="span-text-15a">-</span>
                                    </div>
                                </div>
                                <div class="frame-15d">
                                    <div class="frame-15e">
                                        <div class="line-15f"></div>
                                        <span class="interview">-</span>
                                    </div>
                                    <div class="button">
                                        <div class="button-160" onclick="location.href='<?php echo G5_URL ?>/page/?pid=counsel'">
                                            <span class="service-request">CLASS 상담받기</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            
        </div>
    </div>
    <div class="frame">
        <div class="frame-1">
            
        </div>
        <div class="frame-f">
            <div class="main-menu-icon">
                <div class="school"></div>
            </div>
            <div class="frame-10">
                <div class="title">
                    <span
                        class="title-yeong-eoro-gyeol-gu-reul-mandeul-sigan-keorieo-yuhag-ibenteu">교직원들이 만든 유학원</span >
                    <div
                        class="title-yeong-eoro-gyeol-gu-reul-mandeul-sigan-keorieo-yuhag-ibenteu-11">
                        <span class="mong-seonsaeng-ui-keon-sel-ting-eun">몽선생의 CLASS 매칭
                        </span >
                        <span class="ireonjeom-i-dalla-yo">이런점이 좋아요!</span>
                    </div>
                </div>
                <span class="im-si-deiteo-ibnida-chu-hu-sujeong-yejeong-ibnida-il-sang-ui-soso-han-sungandeul-eul-hamkke-nanu-neun-gonggan-ibnida">
					학생 성향에 맞는 학교와 전공을 매칭하고 <br>
					검증된 학교 중심으로 CLASS를 제안합니다.
				</span >
            </div>
        </div>
    </div>
    <script>
        $(document).ready(function () {
            var $sidebar = $('.sidebar');
            var $main = $('.main');

            // ★ fixed일 때 sidebar를 main 우측에서 12px 떨어지게 하는 함수
            function updateSidebarRight() {
                var mainRect = $main[0].getBoundingClientRect();
                var rightPos = window.innerWidth - mainRect.right - 3;
                $sidebar.css('right', rightPos + 'px');
            }

            var floatPosStart = $sidebar.offset().top - parseFloat($sidebar.css('marginTop').replace(/auto/, 0));
            var floatPosEnd = $main.offset().top + $main.innerHeight() - $sidebar.outerHeight(true);

            $(window).on('scroll resize', function () {
                var y = $(this).scrollTop();

                if (y >= floatPosStart && y <= floatPosEnd) {
                    // fixed 상태
                    $sidebar.removeClass('sidebar-top sidebar-bottom').addClass('sidebar-fixed');
                    updateSidebarRight();  // ★ fixed일 때 우측 위치 업데이트
                } else if (y >= floatPosEnd) {
                    // absolute-bottom 상태
                    $sidebar.removeClass('sidebar-fixed sidebar-top').addClass('sidebar-bottom');
                    $sidebar.css('right', '12px');  // ★ absolute일 때 right:0
                } else {
                    // absolute-top 상태
                    $sidebar.removeClass('sidebar-fixed sidebar-bottom').addClass('sidebar-top');
                    $sidebar.css('right', '12px');  // ★ absolute일 때 right:0
                }
            });

            // 초기 실행
            updateSidebarRight();
        });
    </script>
</div>