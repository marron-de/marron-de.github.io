<?php
/**
 * page file : /theme/THEME_NAME/page/aboutus.html.php
 */
if (!defined('_EYOOM_')) exit;
?>
<link rel="stylesheet" href="../theme/eb4_basic/page/detail_css.css">

<div class="sub-page page-procedure detail page-detail">
    <div class="main-container" data-aos="fade">
        <div class="frame-c">
            <div class="container">
                <div class="frame-d">
                    <span class="sosok-hagi-e">수속하기 >
                    </span >
                    <span class="dapsa">싱가포르 사립대 무료수속 서비스</span>
                </div>
            </div>
        </div>
        <div class="container target-section main">
            <div class="holding_cont">
                <div class="frame-16">
                    <div class="frame-17">
                        <div class="frame-18">
                            <div class="frame-19">
                                <span class="dapsa-jeong-mi-deul-i-deul-eo-gap-ni-da">싱가포르 사립대 무료 수속서비스 안내</span >
                            </div>
                            <div class="frame-1a">
                                <div class="tooltip">
									<button type="button" class="close_btn"></button>
                                    <div class="content">
                                        <span class="supporting-text">마음에 드는 상담을 공유해보세요</span >
                                    </div>
                                    <div class="div"></div>
                                </div>
                                <div class="frame-1b">
                                    <div class="frame-1c">
                                        <div class="share"></div>
                                        <div class="favorite"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <span
                            class="muryo-yuhag-seobiseu-wa-ibsi-keon-sel-ting-kkaji-dayang-han-seobiseu-je-gong">20여개 대학교 전체 수수료 없이 입학수속 가능</span >
                    </div>
                    <div class="frame-1d">
                        <div class="tags-1e">
                            <span class="best">BEST</span>
                        </div>
                        <div class="keulraeseu">
                            <span class="yeong-eo">수수료 0원</span>
                        </div>
                        <div class="frame-1f">
                            <span class="text-16">교직원출신관리</span>
                        </div>
                    </div>
                </div>
                <div class="section-c">
                    <div class="ms title_area class stickybox" id="row-1">
                        <div class="tab sticky">
                            <ul class="tab_tit">
                                <li><a href="#sec01" class="show">서비스 소개</a></li>
                                <li><a href="#sec02">학교 리스트</a></li>
                                <li><a href="#sec03">서비스 포함 내용</a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="classDetail_list">
                        <ul class="panel">
                            <li id="content-1">
								<div class="frame-section ver2 mb" id="sec01">
									<div class="frame-27">
										<span class="field-trip-intro">서비스안내</span>
									</div>
									<span class="field-trip-service">사립대 무료수속 서비스는 어떠한 비용을 지불하지 않고, 보다 쉽고 편리하게 입학준비가 가능합니다. 담당 실장님이 배정되어 1:1관리되고, 서류준비/원서접수 그 외 학교 행정을 관리받을 수 있습니다.</span >
								</div>
								<div class="frame-section ver2">
									<div class="frame-titbox">
										<p class="frame-title">한눈에 보는 컨설팅 기본 정보</p>
									</div>
									<div class="experbox">
										<div class="item">
											<img  src="<?php echo EYOOM_THEME_URL; ?>/img/sub/exper_icon1.png" alt="" class="icon">
											<div class="txtbox">
												<p class="desc">
													<span class="txt">
														싱가포르 전체 사립대학 입학 수속 대행
													</span>
												</p>
											</div>
										</div>
										<div class="item">
											<img  src="<?php echo EYOOM_THEME_URL; ?>/img/sub/exper_icon5.png" alt="" class="icon">
											<div class="txtbox">
												<p class="desc">
													<span class="txt">
														수수료 0원! 추가 비용없이 무료 이용가능
													</span>
												</p>
											</div>
										</div>
										<div class="item">
											<img  src="<?php echo EYOOM_THEME_URL; ?>/img/sub/exper_icon4.png" alt="" class="icon">
											<div class="txtbox">
												<p class="desc">
													<span class="txt">
														원서접수, 서류관리 및 학교 행정 일괄
													</span>
												</p>
											</div>
										</div>
										<div class="item">
											<img  src="<?php echo EYOOM_THEME_URL; ?>/img/sub/exper_icon6.png" alt="" class="icon">
											<div class="txtbox">
												<p class="desc">
													<span class="txt">
														국내 1호, 사립대 교직원 출신이 경험을 통한 수속관리
													</span>
												</p>
											</div>
										</div>
									</div>
								</div>
                                <div class="line-2a"></div>	
                                <div class="frame-section" id="sec02">
                                    <div class="frame-titbox between">
                                        <span class="frame-title">일반 사립대 컨설팅 가능 대학교</span >
                                    </div>
									<div class="school_listbox on">
										<div class="item">
											<div class="imgbox">
												<img src="<?php echo EYOOM_THEME_URL; ?>/img/sub/sch_logo_SIM.png" alt="" class="img">
											</div>
											<div class="txtbox">
												<p class="tit">SIM</p>
												<p class="desc">SIM University</p>
											</div>
										</div>
										<div class="item">
											<div class="imgbox">
												<img src="<?php echo EYOOM_THEME_URL; ?>/img/sub/sch_logo_PSB.png" alt="" class="img">
											</div>
											<div class="txtbox">
												<p class="tit">PSB</p>
												<p class="desc">PSB Academy</p>
											</div>
										</div>
										<div class="item">
											<div class="imgbox">
												<img src="<?php echo EYOOM_THEME_URL; ?>/img/sub/sch_logo_KAPLAN.png" alt="" class="img">
											</div>
											<div class="txtbox">
												<p class="tit">카플란</p>
												<p class="desc">Kaplan University</p>
											</div>
										</div>
										<div class="item">
											<div class="imgbox">
												<img src="<?php echo EYOOM_THEME_URL; ?>/img/sub/sch_logo_MDIS.png" alt="" class="img">
											</div>
											<div class="txtbox">
												<p class="tit">MDIS</p>
												<p class="desc">Management Development Institute of Singapore</p>
											</div>
										</div>
										<div class="item">
											<div class="imgbox">
												<img src="<?php echo EYOOM_THEME_URL; ?>/img/sub/sch_logo_Raffles.png" alt="" class="img">
											</div>
											<div class="txtbox">
												<p class="tit">레플즈</p>
												<p class="desc">Raffles College of Higher Education</p>
											</div>
										</div>
									</div>
                                </div>
                                <div class="frame-section">
                                    <div class="frame-titbox between">
                                        <span class="frame-title">직영 캠퍼스 컨설팅 가능 대학교</span >
                                    </div>
									<div class="school_listbox on">
										<div class="item">
											<div class="imgbox">
												<img src="<?php echo EYOOM_THEME_URL; ?>/img/sub/sch_logo_JCU.png" alt="" class="img">
											</div>
											<div class="txtbox">
												<p class="tit">JCU</p>
												<p class="desc">James Cook University</p>
											</div>
										</div>
										<div class="item">
											<div class="imgbox">
												<img src="<?php echo EYOOM_THEME_URL; ?>/img/sub/sch_logo_Curtin.png" alt="" class="img">
											</div>
											<div class="txtbox">
												<p class="tit">컬틴</p>
												<p class="desc">Curtin University</p>
											</div>
										</div>
										<div class="item">
											<div class="imgbox">
												<img src="<?php echo EYOOM_THEME_URL; ?>/img/sub/sch_logo_KAPLAN.png" alt="" class="img">
											</div>
											<div class="txtbox">
												<p class="tit">카플란</p>
												<p class="desc">Kaplan University</p>
											</div>
										</div>
										<div class="item">
											<div class="imgbox">
												<img src="<?php echo EYOOM_THEME_URL; ?>/img/sub/sch_logo_MDIS.png" alt="" class="img">
											</div>
											<div class="txtbox">
												<p class="tit">MDIS</p>
												<p class="desc">Management Development Institute of Singapore</p>
											</div>
										</div>
										<div class="item">
											<div class="imgbox">
												<img src="<?php echo EYOOM_THEME_URL; ?>/img/sub/sch_logo_Raffles.png" alt="" class="img">
											</div>
											<div class="txtbox">
												<p class="tit">레플즈</p>
												<p class="desc">Raffles College of Higher Education</p>
											</div>
										</div>
									</div>
                                </div>
                                <div class="line-2a"></div>
                                <div class="frame-section ver2" id="sec03">
                                    <span class="frame-title">서비스 포함 내용</span>
									<ul class="service_box">
										<li class="service_item">
											<p class="tit">
												1.  학교 선정 및 전공 상담
											</p>
											<div class="icon"></div>
										</li>
										<li class="service_item">
											<p class="tit">
												2. 입학 서류 안내 및 검토
											</p>
											<div class="icon"></div>
										</li>
										<li class="service_item">
											<p class="tit">
												3.  원서접수, 학사일정 관리 일괄 (3개교 선택)
											</p>
											<div class="icon"></div>
										</li>
										<li class="service_item">
											<p class="tit">
												4.  숙소 안내, 정착 OT 및 현지 생활 가이드
											</p>
											<div class="icon"></div>
										</li>
										<li class="service_item">
											<p class="tit">
												5.  싱가포르 현지 취업 컨설팅 무상제공
											</p>
											<div class="icon"></div>
										</li>
										<li class="service_item">
											<p class="tit">
												6.  비자, 보험 및 행정처리
											</p>
											<div class="icon"></div>
										</li>
									</ul>
									<div class="note_txtbox">
										<p class="note_txt">* 1:1 관리를 원칙으로 하며, 학생 개인 면담 후 추가 되는 수업이 있을 수 있습니다.</p>
										<div class="note_txt">* 카카오톡을 통해 학교 입학까지 개별 수속관리가 이루어집니다.</div>
									</div>
                                </div>
                                <div class="frame-section ver2 mt">
									<div class="frame-titbox between">
                                    	<span class="frame-title">수속하기 전, 담당자와 상담하기</span>
										<a href="https://pf.kakao.com/_fxmxmWV" class="kakao_link" target="_blank">
											<img src="<?php echo EYOOM_THEME_URL; ?>/img/common/ic_kakao.png" alt="" class="icon">
											<span class="txt">빠른 상담</span>
										</a>
									</div>
									<div class="question_wrapper">
										<div class="marquee question_box" data-duration="6">
											<div class="marquee_group">
												<div class="question">
													<p class="txt">무료 수속 서비스내용이 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">비자 신청에 필요한 서류가 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">수속 실장님과 연결하고 싶어요.</p>
												</div>

												<div class="question">
													<p class="txt">사전 입학 심사 가능한가요?</p>
												</div>
												<div class="question">
													<p class="txt">국제학교 입학 티오가 있을까요.</p>
												</div>
												<div class="question">
													<p class="txt">숙소 예약도 도움주시나요?</p>
												</div>
												
												<div class="question">
													<p class="txt">무료 수속 서비스내용이 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">비자 신청에 필요한 서류가 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">수속 실장님과 연결하고 싶어요.</p>
												</div>

												<div class="question">
													<p class="txt">사전 입학 심사 가능한가요?</p>
												</div>
												<div class="question">
													<p class="txt">국제학교 입학 티오가 있을까요.</p>
												</div>
												<div class="question">
													<p class="txt">숙소 예약도 도움주시나요?</p>
												</div>
											</div>
										</div>
										<div class="marquee question_box pc_hide" data-duration="6" data-reverse="reverse">
											<div class="marquee_group">
												<div class="question">
													<p class="txt">무료 수속 서비스내용이 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">비자 신청에 필요한 서류가 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">수속 실장님과 연결하고 싶어요.</p>
												</div>

												<div class="question">
													<p class="txt">사전 입학 심사 가능한가요?</p>
												</div>
												<div class="question">
													<p class="txt">국제학교 입학 티오가 있을까요.</p>
												</div>
												<div class="question">
													<p class="txt">숙소 예약도 도움주시나요?</p>
												</div>
												
												<div class="question">
													<p class="txt">무료 수속 서비스내용이 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">비자 신청에 필요한 서류가 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">수속 실장님과 연결하고 싶어요.</p>
												</div>

												<div class="question">
													<p class="txt">사전 입학 심사 가능한가요?</p>
												</div>
												<div class="question">
													<p class="txt">국제학교 입학 티오가 있을까요.</p>
												</div>
												<div class="question">
													<p class="txt">숙소 예약도 도움주시나요?</p>
												</div>
											</div>
										</div>
										<div class="marquee question_box pc_hide" data-duration="6">
											<div class="marquee_group">
												<div class="question">
													<p class="txt">무료 수속 서비스내용이 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">비자 신청에 필요한 서류가 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">수속 실장님과 연결하고 싶어요.</p>
												</div>

												<div class="question">
													<p class="txt">사전 입학 심사 가능한가요?</p>
												</div>
												<div class="question">
													<p class="txt">국제학교 입학 티오가 있을까요.</p>
												</div>
												<div class="question">
													<p class="txt">숙소 예약도 도움주시나요?</p>
												</div>

												<div class="question">
													<p class="txt">무료 수속 서비스내용이 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">비자 신청에 필요한 서류가 궁금해요.</p>
												</div>
												<div class="question">
													<p class="txt">수속 실장님과 연결하고 싶어요.</p>
												</div>

												<div class="question">
													<p class="txt">사전 입학 심사 가능한가요?</p>
												</div>
												<div class="question">
													<p class="txt">국제학교 입학 티오가 있을까요.</p>
												</div>
												<div class="question">
													<p class="txt">숙소 예약도 도움주시나요?</p>
												</div>
											</div>
										</div>
									</div>
									<!-- <div class="swiper question_swiper question_box">
										<div class="swiper-wrapper">
											<div class="swiper-slide question">
												<p class="txt">질문 내용이 들어갑니다.</p>
											</div>
											<div class="swiper-slide question">
												<p class="txt">질문 내용이 들어갑니다질문 내용이 들어갑니다.</p>
											</div>
											<div class="swiper-slide question">
												<p class="txt">질문 내용이 들어갑니다.</p>
											</div>

											<div class="swiper-slide question">
												<p class="txt">질문 내용이 들어갑니다.</p>
											</div>
											<div class="swiper-slide question">
												<p class="txt">질문 내용이 들어갑니다질문 내용이 들어갑니다.</p>
											</div>
											<div class="swiper-slide question">
												<p class="txt">질문 내용이 들어갑니다.</p>
											</div>
											
											<div class="swiper-slide question">
												<p class="txt">질문 내용이 들어갑니다.</p>
											</div>
											<div class="swiper-slide question">
												<p class="txt">질문 내용이 들어갑니다질문 내용이 들어갑니다.</p>
											</div>
											<div class="swiper-slide question">
												<p class="txt">질문 내용이 들어갑니다.</p>
											</div>
										</div>
									</div> -->
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="floating_cont sidebar">
                <div class="floating_wrap">
                    <div class="sseom-neol ver2">
                        <div class="mal-lei-si-a-dae-hag prcd01"></div>
                    </div>
                    <div class="frame-14f">
                        <div class="frame-150">
                            <div class="tab_navbox notab frame-151">
                                <div class="tab_nav on2 frame-152">무료 수속</div>
                            </div>
                            <div class="frame-157 default_ver">
                                <div class="frame-158">
                                    <div class="frame-159">
                                        <span class="span-text-15a">0원</span>
                                        <div class="frame-15b">
                                            <span class="time-15c">대학 원서비 별도 납부</span>
                                        </div>
                                    </div>
									<p class="desc">
										입학수속과 정착까지 원스탑 서비스 제공
									</p>
                                </div>
                                <div class="frame-15d">
                                    <div class="frame-15e">
                                        <span class="interview">
                                            사립대 무료수속 서비스는 어떠한 비용을 지불하지 않고, 보다 쉽고 편리하게 입학준비가 가능합니다. 담당 실장님이 배정되어 1:1관리되고, 서류준비/원서접수 그 외 학교 행정을 관리받을 수 있습니다.
                                        </span >
                                    </div>
                                    <div class="button">
                                        <div class="button-160" onclick="location.href='<?php echo G5_URL ?>/page/?pid=counsel'">
                                            <span class="service-request">서비스 요청하기</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            
        </div>
    </div>
    <div class="frame">
        <div class="frame-1">
            
        </div>
        <div class="frame-f">
            <div class="main-menu-icon">
                <div class="school"></div>
            </div>
            <div class="frame-10">
                <div class="title">
                    <span
                        class="title-yeong-eoro-gyeol-gu-reul-mandeul-sigan-keorieo-yuhag-ibenteu">교직원들이 만든 유학원</span >
                    <div
                        class="title-yeong-eoro-gyeol-gu-reul-mandeul-sigan-keorieo-yuhag-ibenteu-11">
                        <span class="mong-seonsaeng-ui-keon-sel-ting-eun">몽선생의 컨설팅은
                        </span >
                        <span class="ireonjeom-i-dalla-yo">이런점이 달라요!</span>
                    </div>
                </div>
                <span
                    class="im-si-deiteo-ibnida-chu-hu-sujeong-yejeong-ibnida-il-sang-ui-soso-han-sungandeul-eul-hamkke-nanu-neun-gonggan-ibnida">임시 데이터입니다. 추후 수정 예정입니다.<br/>일상의 소소한 순간들을 함께 나누는 공간입니다.</span >
            </div>
        </div>
    </div>
    <script>
        $(document).ready(function () {
            var $sidebar = $('.sidebar');
            var $main = $('.main');

            // ★ fixed일 때 sidebar를 main 우측에서 12px 떨어지게 하는 함수
            function updateSidebarRight() {
                var mainRect = $main[0].getBoundingClientRect();
                var rightPos = window.innerWidth - mainRect.right - 3;
                $sidebar.css('right', rightPos + 'px');
            }

            var floatPosStart = $sidebar.offset().top - parseFloat($sidebar.css('marginTop').replace(/auto/, 0));
            var floatPosEnd = $main.offset().top + $main.innerHeight() - $sidebar.outerHeight(true);

            $(window).on('scroll resize', function () {
                var y = $(this).scrollTop();

                if (y >= floatPosStart && y <= floatPosEnd) {
                    // fixed 상태
                    $sidebar.removeClass('sidebar-top sidebar-bottom').addClass('sidebar-fixed');
                    updateSidebarRight();  // ★ fixed일 때 우측 위치 업데이트
                } else if (y >= floatPosEnd) {
                    // absolute-bottom 상태
                    $sidebar.removeClass('sidebar-fixed sidebar-top').addClass('sidebar-bottom');
                    $sidebar.css('right', '12px');  // ★ absolute일 때 right:0
                } else {
                    // absolute-top 상태
                    $sidebar.removeClass('sidebar-fixed sidebar-bottom').addClass('sidebar-top');
                    $sidebar.css('right', '12px');  // ★ absolute일 때 right:0
                }
            });

            // 초기 실행
            updateSidebarRight();
        });
    </script>
</div>