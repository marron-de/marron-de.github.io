...
// tab function
const tabs_swiper = new Swiper('.tabs_swiper:not(.ver2)', {
	observer: true, 	
	observeParents: true, 
...
const tabs_swiper2 = new Swiper('.tabs_swiper.ver2', {
	observer: true, 	
	observeParents: true, 
...
$(document).ready(function () {
	$(".common_banner .clsoe_btn").click(function () {
		$(".common_banner").addClass("hide")
	})
});
...
$(document).ready(function () {
	...
	//   $("body").addClass("hidden"); // 페이지 이동후 뒤로가기 시 오류발생 -> 제거
...
$(function () {
  ...
  const $clone = $lineWrap.clone();
  $clone.appendTo('.bgbox');

  let pos = 0;
  const speed = 0.5; // px 단위
  const totalHeight = $lineWrap.height();
  ...
  function loopScroll() {
    ...
    $lineWrap.css('transform', `translateY(${pos}px)`);
    $clone.css('transform', `translateY(${pos + totalHeight}px)`);
...
if (window.innerWidth <= 1080) {
	$('.ors2_swiper').each(function () {
		const swiperEl = $(this)[0];
