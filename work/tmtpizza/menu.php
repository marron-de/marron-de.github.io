<div class="wrap menu_wrap">

	<!-- common header -->
	<?php include_once(G5_THEME_PATH.'/common_header.php'); ?>

	<div class="lnb_wrap sub_02_lnb">
		<div class="lnb_inner">
			<p class="lnb_tit">MENU</p>
		</div>
	</div>

	<!-- container -->
	<div class="container sub_container inner">

		<div class="tab_wrap menu_cont">
			
			<ul class="tab_navbox">
				<li class="tab_nav on" data-category="pizza">피자</li>
				<li class="tab_nav" data-category="set">세트</li>
				<li class="tab_nav" data-category="side">사이드</li>
				<li class="tab_nav" data-category="drink">음료&amp;소스</li>
			</ul>
			
			<div class="tab_contbox tmt_menu_box menu-pages">
				
				<ul class="tab_cont on tmt_list_box menu-page" data-page="pizza">
					<!-- 01 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_01_tmt.png" alt="티엠티 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">티엠티 피자</p>
								<p class="menu_name_en">TMT PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>티엠티 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									&#9733; T.M.T PIZZA의 시그니처 메뉴 &#9733; <br>
									<br>
									토핑UP, 맛도 UP 맛있는 토핑이 모두 올라간 피자<br>
									남녀노소, 강아지, 고양이까지 누구나 좋아하는 메뉴
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
						<ul class="menu_effect">
							<li class="best_eff">
								<div class="eff_box">
									<img src="<?php echo G5_THEME_URL ?>/img/main/best_effect.svg" alt="베스트 메뉴">
									<p class="eff_txt">BEST</p>
								</div>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_02_egg.png" alt="에그마요 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">에그마요 피자</p>
								<p class="menu_name_en">EGG MAYO PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>에그마요 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
								황금 스프레드가 듬뿍 올라간 <br>
								포만감 가득~ 부드러운 피자
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
						<ul class="menu_effect">
							<li class="best_eff">
								<div class="eff_box">
									<img src="<?php echo G5_THEME_URL ?>/img/main/new_effect.svg" alt="신메뉴">
									<p class="eff_txt">NEW</p>
								</div>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_03_nacho.png" alt="나쵸 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">나쵸 피자</p>
								<p class="menu_name_en">NACHO PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>나쵸 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									나쵸로 바삭한 식감은 UP! 직원들 &#9733;최애메뉴&#9733;<br>
									요즘 MZ들 세대를 겨냥하여 만든 트렌디한 메뉴<br>
									나쵸를 올려 바삭바삭한 식감에<br>
									할라피뇨와 맛있는 토핑이 더해진 피자.
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_04_bulgogi.png" alt="소불고기 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">소불고기 피자</p>
								<p class="menu_name_en">BEEF BUIGOGI PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>소불고기 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									한국인이라면 불고기를 싫어할 수 없잖아요?<br>
									돼지고기가 아닌 진짜 소고기로 만든 불고기<br>
									&quot;돼지고기가 아닌 진짜 소.고.기&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<!-- 02 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_05_grilled.png" alt="그릴드 스테이크 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">그릴드 스테이크 피자</p>
								<p class="menu_name_en">GRILLED STEAK PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>그릴드<br> 스테이크 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									프.리.미.엄 라인 첫번째 메뉴의 대반전!<br>
									부드러운 스테이크 고기와 감자, 할라피뇨를 더해<br>
									느끼함을 잡아주며 적당한 가격으로 부담없는 메뉴!<br>
									&quot;이가격에 프리미엄을 먹을 수 있다니..!&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_06_hot_bul.png" alt="핫불고기 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">핫불고기 피자</p>
								<p class="menu_name_en">HOT BULGOGI PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>핫불고기 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									소불고기에 매운맛을 추가한<br>
									매운 불고기 핫불고기 피자<br>
									&quot;불고기에 매운맛 추가인데 맛 없을수가..&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_07_hot_shrimp.png" alt="핫칠리 쉬림프 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">핫칠리 쉬림프 피자</p>
								<p class="menu_name_en">HOT CHILI SHRIMP PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>핫칠리<br>쉬림프 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									맛있는 베이컨과 칠리소스에 버무려진 새우가 쏘옥<br>
									&quot;베이컨 위에 칠리소스를 머금은 새우가<br>
									입 안에서 탭댄스를 춘다..&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_08_bacon_shrimp.png" alt="베이컨 쉬림프 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">베이컨 쉬림프 피자</p>
								<p class="menu_name_en">BACON SHRIMP PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>베이컨<br>쉬림프 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									매콤새콤달콤한 칠리소스를 머금은 새우가<br>
									한가득 올라간 피자<br>
									&quot;매콤새콤달콤한 칠리소스를 머금은 새우가 입안에서<br>
									힙합댄스를 춘다..&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<!-- 03 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_09_spicy_chicken.png" alt="스파이시 핫치킨 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">스파이시 핫치킨 피자</p>
								<p class="menu_name_en">SPICY HOT CHICKEN PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>스파이시<br>핫치킨 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									매운 양념을 버무려 재운 닭고기와<br>
									할라피뇨가 올라간 피자<br>
									매운거 좋아하시는 분들이 찾아주시는 피자 메뉴<br>
									&quot;매운 음식 잘드시는분 강.력.추.천!&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_10_honey_corn.png" alt="허니 콘치즈 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">허니 콘치즈 피자</p>
								<p class="menu_name_en">HONEY CORN CHEESE PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>허니 콘치즈 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									달달한 연유 드레싱으로 아이들에게 인기만점 메뉴<br>
									&quot;톡톡 터지는 옥수수콘과 달콤한 연유가 만난<br>달콤한 피자&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_11_pineapple.png" alt="통하와이안 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">통하와이안 피자</p>
								<p class="menu_name_en">PINEAPPLE PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>통하와이안 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									파인애플이 통으로 올라간 상큼 발랄한 피자<br>
									&quot;파인애플 피자가 호불호가 갈린다?<br>
									이해할 수 없습니다!&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_12_bacon_potato.png" alt="베이컨 포테이토 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">베이컨 포테이토 피자</p>
								<p class="menu_name_en">BACON POTATO PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>베이컨<br>포테이토 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									듬뿍 담긴 감자 위에 베이컨이<br>
									쫘악 깔린 피자<br>
									마요네즈의 상식은 버려라!<br>
									&quot;듬뿍 담긴 감자에 베이컨도 촤악~!&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<!-- 04 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_13_hot_pepperoni.png" alt="할라 핫 페퍼로니 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">할라 핫 페퍼로니 피자</p>
								<p class="menu_name_en">HOT PEPPERONI PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>할라<br>핫 페퍼로니 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									매운소스 베이스에 할라피뇨를 더한<br>
									매콤한 페퍼로니 피자<br>
									&quot;매운 한국인의 입맛에 맞춘 메뉴&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_14_real_cheese.png" alt="리얼 치즈 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">리얼 치즈 피자</p>
								<p class="menu_name_en">REAL CHEESE PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>리얼 치즈 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									T.M.T PIZZA의<br>
									고급진 치즈맛을 느낄 수 있는 피자.<br>
									&quot;자연산 100% 진짜 치즈&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_15_vegetable.png" alt="싱싱 야채 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">싱싱 야채 피자</p>
								<p class="menu_name_en">VEGETABLE PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>싱싱 야채 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									싱싱한 야채가 메인 재료인 피자<br>
									다이어트 하시는 분들에게 필수 메뉴<br>
									&quot;다이어트? 티엠티가 도와드리겠습니다.&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_16_potato.png" alt="포테이토 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">포테이토 피자</p>
								<p class="menu_name_en">POTATO PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>포테이토 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									드레싱 소스로 한층 맛을 업그레이드<br>
									담백한걸 좋아하시는 분들에게 인기가 많은 메뉴<br>
									&quot;감자와 맛있는 소스의 만남&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<!-- 05 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_17_sweet_potato.png" alt="달콤 고구마무스 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">달콤 고구마무스 피자</p>
								<p class="menu_name_en">SWEET POTATO PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>달콤<br>고구마무스 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									달달한 고구마 무스가 듬뿍 들어간 피자<br>
									고구마를 좋아하시는 분들에게 강력 추천!<br>
									&quot;혹시 고구마를 좋아 하시나요?<br>
									좋아하시는 만큼 드리겠습니다.&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_18_bacon_cheddar.png" alt="베이컨 체다 치즈 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">베이컨 체다 치즈 피자</p>
								<p class="menu_name_en">BACON CHEDDAR CHEESE PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>베이컨<br>체다 치즈 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									짭쪼롬한 베이컨과 체다치즈의 놀라운 조화 <br>
									단짠단짠의 짠을 맡고 있는 효자 메뉴 <br>
									&quot;오늘 밤 맥주 한 잔~?&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_19_pepperoni.png" alt="꽉채운 페퍼로니 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">꽉채운 페퍼로니 피자</p>
								<p class="menu_name_en">PEPPERONI PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>꽉채운<br>페퍼로니 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									전세계 판매 1위 메뉴 페퍼로니 피자<br>
									전체 메뉴별 티엠티 피자 다음으로 수요층이 많은 메뉴<br>
									&quot;꽉꽉 빈틈없이 채운 페퍼로니로 뉴욕을 느껴봐요&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_20_combination.png" alt="콤비네이션 피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">콤비네이션 피자</p>
								<p class="menu_name_en">COMBINATION PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>콤비네이션 피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									고기 토핑이 듬뿍 들어간 콤비네이션!<br>
									피자 메뉴의 정석!<br>
									&quot;베이직 is 베스트!&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<!-- 06 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_21_thin_gorgonzola.png" alt="고르곤졸라 씬피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">고르곤졸라 씬피자</p>
								<p class="menu_name_en">GORGONZOLA THIN PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>고르곤졸라 씬피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									바삭한 씬도우에 꿀을 찍어서 먹으면 게임 끝<br>
									&quot;심지어 꿀도 맛있는 고르곤졸라.&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/menu_22_thin_basil.png" alt="바질 베이컨 씬피자"></li>
							<li class="menu_text">
								<p class="menu_name_kr">바질 베이컨 씬피자</p>
								<p class="menu_name_en">BASIL BACON THIN PIZZA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>바질 베이컨 씬피자</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									바질페스토로 씬 베이스에<br>
									바질바질향연으로 만든 이색피자<br>
									&quot;바질베이컨 씬 먹으면 입안이 바질바질해&quot;
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>10,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>10,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<!-- menu_list END -->
				</ul>
				
				<ul class="tab_cont tmt_list_box menu-page" data-page="set">
					<!-- 01 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/set_01_spaghetti.png" alt="스파게티 세트"></li>
							<li class="menu_text">
								<p class="menu_name_kr">스파게티 세트</p>
								<p class="menu_name_en">SPAGHETTI SET</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>스파게티 세트</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									피자+스파게티+코카콜라500ml
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>17,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>24,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>28,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
						<ul class="menu_effect">
							<li class="best_eff">
								<div class="eff_box">
									<img src="<?php echo G5_THEME_URL ?>/img/main/best_effect.svg" alt="베스트 메뉴">
									<p class="eff_txt">BEST</p>
								</div>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/set_02_shrimp.png" alt="새우링 세트"></li>
							<li class="menu_text">
								<p class="menu_name_kr">새우링 세트</p>
								<p class="menu_name_en">SHRIMP RING SET</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>새우링 세트</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									피자+새우링6ea+코카콜라500ml
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>17,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>24,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>28,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
						<ul class="menu_effect">
							<li class="best_eff">
								<div class="eff_box">
									<img src="<?php echo G5_THEME_URL ?>/img/main/new_effect.svg" alt="신메뉴">
									<p class="eff_txt">NEW</p>
								</div>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/set_03_cheese_stick.png" alt="치즈스틱 세트"></li>
							<li class="menu_text">
								<p class="menu_name_kr">치즈스틱 세트</p>
								<p class="menu_name_en">CHEESE STICK SET</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>치즈스틱 세트</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									피자+치즈스틱6ea+코카콜라500ml
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>17,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>24,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>28,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/set_04_hotwing.png" alt="핫윙봉 세트"></li>
							<li class="menu_text">
								<p class="menu_name_kr">핫윙봉 세트</p>
								<p class="menu_name_en">HOT WING BONG SET</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>스핫윙봉 세트</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									피자+핫윙봉6ea+코카콜라500ml
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>17,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>24,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>28,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<!-- 02 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/set_05_smoke_chicken.png" alt="훈제치킨 세트"></li>
							<li class="menu_text">
								<p class="menu_name_kr">훈제치킨 세트</p>
								<p class="menu_name_en">SMOKED CHICKEN SET</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>훈제치킨 세트</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									피자+훈제치킨 반마리+코카콜라500ml
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>21,900<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>26,900<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>30,900<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
				</ul>
				
				<ul class="tab_cont tmt_list_box menu-page" data-page="side">
					<!-- 01 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/side_01_cheese_oven.png" alt="치즈 오븐스파게티"></li>
							<li class="menu_text">
								<p class="menu_name_kr">치즈 오븐스파게티</p>
								<p class="menu_name_en">CHEESE OVEN SPAGHETTI</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>치즈 오븐스파게티</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>5,500<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
						<ul class="menu_effect">
							<li class="best_eff">
								<div class="eff_box">
									<img src="<?php echo G5_THEME_URL ?>/img/main/best_effect.svg" alt="베스트 메뉴">
									<p class="eff_txt">BEST</p>
								</div>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/side_02_bulgogi.png" alt="불고기 오븐스파게티"></li>
							<li class="menu_text">
								<p class="menu_name_kr">불고기 오븐스파게티</p>
								<p class="menu_name_en">BULGOGI OVEN SPAGHETTI</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>불고기 오븐스파게티</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>6,500<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/side_03_bacon_cream.png" alt="베이컨크림 오븐스파게티"></li>
							<li class="menu_text">
								<p class="menu_name_kr">베이컨크림 오븐스파게티</p>
								<p class="menu_name_en">BACON CREAM OVEN SPAGHETTI</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>베이컨크림<br> 오븐스파게티</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>6,500<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/side_04_basil_cream.png" alt="바질크림 오븐스파게티"></li>
							<li class="menu_text">
								<p class="menu_name_kr">바질크림 오븐스파게티</p>
								<p class="menu_name_en">BASIL CREAM OVEN SPAGHETTI</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>바질크림 오븐스파게티</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>7,000<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
						<ul class="menu_effect">
							<li class="best_eff">
								<div class="eff_box">
									<img src="<?php echo G5_THEME_URL ?>/img/main/new_effect.svg" alt="신메뉴">
									<p class="eff_txt">NEW</p>
								</div>
							</li>
						</ul>
					</li>
					<!-- 02 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/side_05_smoked_chicken.png" alt="훈제치킨"></li>
							<li class="menu_text">
								<p class="menu_name_kr">훈제치킨</p>
								<p class="menu_name_en">SMOKED CHICKEN</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>훈제치킨</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									반마리 - 머스터드 1 증정 <br>
									한마리 - 머스터드 2 증정
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>7,400<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>14,000<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/side_06_hotwing.png" alt="핫윙&봉"></li>
							<li class="menu_text">
								<p class="menu_name_kr">핫윙&amp;봉</p>
								<p class="menu_name_en">HOT WIHNG &amp; BONG</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>핫윙&amp;봉</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									4ea - 4,000원<br>
									6ea - 5,500원<br>
									8ea - 7,000원
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>4,000<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>5,500<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>7,000<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/side_07_shrimp_ring.png" alt="새우링"></li>
							<li class="menu_text">
								<p class="menu_name_kr">새우링</p>
								<p class="menu_name_en">SHRIMP RING</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>새우링</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									4ea - 4,000원<br>
									6ea - 5,500원<br>
									8ea - 7,000원
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>4,000<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>5,500<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>7,000<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/side_08_cheese_stick.png" alt="치즈스틱"></li>
							<li class="menu_text">
								<p class="menu_name_kr">치즈스틱</p>
								<p class="menu_name_en">CHEESE STICK</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>치즈스틱</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									4ea - 4,000원<br>
									6ea - 5,500원<br>
									8ea - 7,000원
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_s m_cip">S</span>
										<p>4,000<span>원</span></p>
									</li>
									<li>
										<span class="col_r m_cip">R</span>
										<p>5,500<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>7,000<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
				</ul>
				
				<ul class="tab_cont tmt_list_box menu-page" data-page="drink">
					<!-- 01 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/drink_01_coca.png" alt="코키콜라"></li>
							<li class="menu_text">
								<p class="menu_name_kr">코카콜라</p>
								<p class="menu_name_en">COCA COLA</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>코카콜라</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>2,000<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>2,500<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/drink_02_sprite.png" alt="스프라이트"></li>
							<li class="menu_text">
								<p class="menu_name_kr">스프라이트</p>
								<p class="menu_name_en">SPRITE</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>스프라이트</h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>2,000<span>원</span></p>
									</li>
									<li>
										<span class="col_l m_cip">L</span>
										<p>2,500<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/drink_03_pepsi.png" alt="펩시제로"></li>
							<li class="menu_text">
								<p class="menu_name_kr">펩시제로</p>
								<p class="menu_name_en">PEPSI ZERO</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>펩시제로<h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>2,000<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/sauce_01_hot.png" alt="티엠티피자 핫소스"></li>
							<li class="menu_text">
								<p class="menu_name_kr">티엠티피자 핫소스</p>
								<p class="menu_name_en">TMT PIZZA HOT SAUCE</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>티엠티피자 핫소스<h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>2,000<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<!-- 02 -->
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/sauce_02_parmasan.png" alt="티엠티피자 파마산골드"></li>
							<li class="menu_text">
								<p class="menu_name_kr">티엠티피자 파마산골드</p>
								<p class="menu_name_en">TMT PIZZA PARMASAN GOLD</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>티엠티피자<br>파마산골드<h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>2,000<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/sauce_03_garlic.png" alt="티엠티피자 갈릭디핑소스"></li>
							<li class="menu_text">
								<p class="menu_name_kr">티엠티피자 갈릭디핑소스</p>
								<p class="menu_name_en">TMT PIZZA GARLIC DIPPING SAUCE</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>티엠티피자<br>갈릭디핑소스<h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>2,000<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/sauce_04_yogurt.png" alt="티엠티피자 요거트소스"></li>
							<li class="menu_text">
								<p class="menu_name_kr">티엠티피자 요거트소스</p>
								<p class="menu_name_en">TMT PIZZA YOGURT SAUCE</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>티엠티피자<br>요거트소스<h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>2,000<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
					<li class="menu-item">
						<ul class="menu_list">
							<li class="menu_img"><img src="<?php echo G5_THEME_URL ?>/img/sub_02/sauce_05_pickle.png" alt="티엠티피자 오이피클"></li>
							<li class="menu_text">
								<p class="menu_name_kr">티엠티피자 오이피클</p>
								<p class="menu_name_en">TMT PIZZA PICKLE</p>
							</li>
						</ul>
						<ul class="menu_info">
							<li class="menu_in_tit">
								<h2>티엠티피자 오이피클<h2>
							</li>
							<li class="menu_in_txt">
								<p>
									-
								</p>
							</li>
							<li class="menu_in_pirce">
								<ul class="menu_pirce">
									<li>
										<span class="col_r m_cip">R</span>
										<p>2,000<span>원</span></p>
									</li>
								</ul>
							</li>
						</ul>
					</li>
				</ul>
			</div>
		</div>

	</div>

	<!-- common footer -->
	<?php include_once(G5_THEME_PATH.'/common_footer.php'); ?>
	
</div>